<template>
      ...
      >
        ...
        >
          <div class="relative aspect-square mb-2 bg-transparent rounded-lg overflow-hidden flex items-center justify-center">
            <img
              v-if="folder.src && !folder.imageError"
            ...
            >
              ...
              <div v-if="!selectionMode" class="px-3 pb-3 flex gap-2 mt-auto">
                ...
                <template v-if="!isTrashFolder">
                  ...
                  <!-- Редактирование изображений отключено в Media2 -->
                  <!-- Переместить -->
                  <button
  ...
  <!-- Image Editor removed - not included in Media2 package -->
...
import { apiGet, apiPost, apiDelete, apiPut, setApiBase } from '../utils/api.js'
...
export default {
  ...
  emits: ['file-selected'],
  setup(props, { emit }) {
    console.log('[Media2] setup() вызван')
    
    // Устанавливаем базовый URL API
    setApiBase(props.apiBaseUrl)
    ...
    const uploadFileWithProgress = (file, folderId, onProgress) => {
      return new Promise((resolve, reject) => {
        ...
        // Открываем соединение (без токенов авторизации)
        const apiBase = props.apiBaseUrl || '/api/v1'
        xhr.open('POST', `${apiBase}/media`)
        
        // Устанавливаем заголовки (без авторизации)
    ...
    // Редактирование изображений отключено в Media2 (ImageEditor не включен в пакет)
