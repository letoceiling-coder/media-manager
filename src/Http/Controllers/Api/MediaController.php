...
namespace LetoceilingCoder\MediaManager\Http\Controllers\Api;
...
use LetoceilingCoder\MediaManager\Http\Requests\StoreMediaRequest;
use LetoceilingCoder\MediaManager\Http\Resources\MediaResource;
use LetoceilingCoder\MediaManager\Models\Folder;
use LetoceilingCoder\MediaManager\Models\Media;
...
class MediaController extends Controller
{
    ...
    public function index(Request $request): AnonymousResourceCollection
    {
        ...
        // Для корзины показываем все файлы, для обычных папок исключаем удаленные
        if (!$isTrashFolder) {
        ...
        // Пагинация
        $perPage = (int) $request->get('per_page', config('media2.pagination.per_page', 20));
        $perPageMax = config('media2.pagination.per_page_max', 100);
        ...
        if ($perPage < 1) {
            $perPage = config('media2.pagination.per_page', 20);
        }
    ...
    {
        $media = Media::with(['folder'])->findOrFail($id);
    ...
    public function update(Request $request, string $id): JsonResponse
    {
        ...
        try {
            ...
            // Перемещаем файл если папка изменилась
