...
use LetoceilingCoder\MediaManager\Http\Filters\FolderFilter;
use LetoceilingCoder\MediaManager\Http\Requests\StoreFolderRequest;
use LetoceilingCoder\MediaManager\Http\Resources\FolderResource;
...
class FolderController extends Controller
{
    ...
    public function destroy(Request $request, string $id): JsonResponse
    {
        try {
            ...
            // Безвозвратное удаление если force=1
            if ($request->has('force') && $request->get('force') == '1') {
                DB::beginTransaction();

                $deleteFolderPermanently = function($folderToDelete) use (&$deleteFolderPermanently) {
                    $files = Media::where('folder_id', $folderToDelete->id)->get();
                    foreach ($files as $file) {
                        $metadata = $file->metadata ? json_decode($file->metadata, true) : [];
                        $filePath = public_path($metadata['path'] ?? ($file->disk . '/' . $file->name));
